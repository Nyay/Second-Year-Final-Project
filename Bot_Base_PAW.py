...
WEBHOOK_URL_BASE = "https://{}:{}".format(Bot_Brain.WEBHOOK_HOST, Bot_Brain.WEBHOOK_PORT)
WEBHOOK_URL_PATH = "/{}/".format(Bot_Brain.TOKEN)
bot = telebot.TeleBot(Bot_Brain.TOKEN,  threaded=False)
bot.remove_webhook()
...
def start(message):
    bot.send_message(message.chat.id, 'Привет! Я бот для снятия омонимии в предложениях. Надеюсь, что когда-нибудь'
                                      ...
                                      ' узнать как я работаю, пиши /help. Так ты узнаешь как использовать команды.\n'
                                      'Удачи, надеюсь тебе понравится! Со всеми предложениям, пиши моему создателю '
                                      '@sorry_im_neet')
...
def helper(message):
    bot.send_message(message.chat.id, 'Так-так. Сейчас все расскажу.\n'
                                      'Для начала работы необходимо загрузить сообщение, которое ты собираешься '
                                      'анализировть.\n\n'
                                      '/new_line (текст через пробел) – Добавляет предложение с которым мне потом '
                                      'предстоит работать. Обязательно не забудь пробел, иначе я могу не понять тебя\n\n'
                                      ...
                                      'то можно переходить сразу к загрузке в мою память.\n\n'
                                      '/commit – Загрузить разбор в файл который хранится у меня. Возможно... '
                                      ...
                                      'прописываем вот эту команду:\n\n'
                                      '/select – Я покажу тебе слова у которых я не смогла определиться с разбором.'
                                      ' После того, как ты выбрал слово, тебе будут представлены мои варианты разбора'
                                      ...
                                      '/add (слово) (разбор) – Это на всякий случай, если слово должно иметь не один разбор.'
                                      ' Для этого необходимо написать команду, через пробел слово и еще через пробел сам разбор.'
                                      ' Я надеюсь, что это тебе не пригодится, но такая функция тоже есть.\n\n'
                                      '/show_all – Это команда для вывода всех разборов в моей памяти на экран.\n\n'
                                      'Четь не забыла! Если тебе не нравится разбор,'
                                      ' тогда просто напиши мне это слово, а затем правильный разбор. Я все перепишу!\n'
                                      'По всем вопросам ты также можешь обращаться к создателю @sorry_im_neet ^^')
...
def add(message):
    ...
    if len(items) == 2 and len(data) > 2:
        if items[0] in data:
            ...
            bot.send_message(message.chat.id, 'Такого слова в моей памяти нет(99((9(9(9')
    else:
...
def analise(message):
    if len(problems) == 0:
        bot.send_message(message.chat.id, 'Мне неичего выбирать, все вроде бы разобрано...'
                                          ' Но если вдруг ты хочешь записать другой разбор, то просто напиши это слово '
...
def show_all(message):
    file = open('/home/IAmAgainstHomonymyBot/mysite/result.txt', 'r', encoding='UTF-8')
    text = file.read()
    file.close()
...
def commit(message):
    if len(data) == 1:
        ...
        file = open('/home/IAmAgainstHomonymyBot/mysite/result.txt', 'a', encoding='UTF-8')
        lines = []
        for word in data:
            if word != 'line':
                for el in data[word]:
...
def designation(message):
    bot.send_message(message.chat.id, 'Давай я тебя познакомлю с оформленем морфологического рабора. '
...
def job(message):
    try:
        ...
        elif message.text[0] == 'A' or 'S' or 'I' or 'C' or 'N' or 'V' or 'P':
            data[current_word[0]] = []
